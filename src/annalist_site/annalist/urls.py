...
urlpatterns = patterns('',
    ...
    url(r'^c/(?P<coll_id>\w{0,32})/types/(?P<type_id>\w{0,32})/$',
                            RecordTypeEditView.as_view(),
                            name='AnnalistRecordTypeAccessView'),
    url(r'^c/(?P<coll_id>\w{0,32})/types/!(?P<action>new)$',
                            RecordTypeEditView.as_view(),
                            name='AnnalistRecordTypeNewView'),
    url(r'^c/(?P<coll_id>\w{0,32})/types/(?P<type_id>\w{0,32})/!(?P<action>copy)$',
                            RecordTypeEditView.as_view(),
                            name='AnnalistRecordTypeCopyView'),
    url(r'^c/(?P<coll_id>\w{0,32})/types/(?P<type_id>\w{0,32})/!(?P<action>edit)$',
                            RecordTypeEditView.as_view(),
                            name='AnnalistRecordTypeEditView'),
    url(r'^c/(?P<coll_id>\w{0,32})/types/!delete_confirmed$',
                            RecordTypeDeleteConfirmedView.as_view(),
                            name='AnnalistRecordTypeDeleteView'),

    url(r'^c/(?P<coll_id>\w{0,32})/views/(?P<view_id>\w{0,32})/$',
                            RecordViewEditView.as_view(),
                            name='AnnalistRecordViewAccessView'),

    url(r'^c/(?P<coll_id>\w{0,32})/lists/(?P<list_id>\w{0,32})/$',
