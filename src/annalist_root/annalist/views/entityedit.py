...
class GenericEntityEditView(AnnalistGenericView):
    ...
    def save_entity(
        ...
        *1 special case when type is '_type' or '_coll'
        ...
        #     )
        orig_typeinfo  = viewinfo.entitytypeinfo
        messages       = viewinfo.type_messages
        ...
        if not orig_typeinfo.parent_exists():
        ...
        if save_type_id != orig_type_id:
            # log.info("save_type_id: save_type_id %s"%(save_type_id))
            save_typeinfo = EntityTypeInfo(
                viewinfo.collection, save_type_id, 
                create_typedata=True
                )
        else:
            save_typeinfo = orig_typeinfo
        ...
        # entity_values[ANNAL.CURIE.type_id] = save_type_id
        # entity_values[ANNAL.CURIE.type]    = save_typeinfo.get_type_uri() # or save_typeinfo.entityclass._entitytype
        ...
        if action == "new":
            ...
            if save_coll_id != orig_coll_id:
                ...
                err_vals, entity_values = self.save_update(viewinfo, save_typeinfo, save_entity_id, entityformvals)
        ...
        #             typeinfo, orig_entity_id, save_typeinfo, save_entity_id, entity_values
        #             )
    ...
    def save_copy(self, viewinfo, 
        ...
        if not err_vals:
            ...
            #@@ temporary for testing - then incorporate logic below and fix.
            err_vals = self.create_update_entity(save_typeinfo, save_entity_id, entity_values)
        # return self.copy_entity(
        #     orig_typeinfo, orig_entity_id, 
        #     save_typeinfo, save_entity_id, 
        #     entity_values
        #     )
    ...
    def copy_entity(self,
        ...
        """
        log.info(
            "copy_entity old: %s/%s, new: %s/%s, vals: %r"%
              ( old_typeinfo.type_id, old_entity_id, 
                new_typeinfo.type_id, new_entity_id, 
                entity_values
              )
    ...
    def save_invoke_task(self, 
        ...
        if task_id == entitytypeinfo.TASK_ID+"/Define_view_list":
            ...
            view_entity.setdefault(RDFS.CURIE.label,   message.TYPE_VIEW_LABEL%type_label)
            view_entity.setdefault(RDFS.CURIE.comment, message.TYPE_VIEW_COMMENT%type_label)
            view_entity._save()
            ...
            list_entity.setdefault(RDFS.CURIE.label,   message.TYPE_LIST_LABEL%type_label)
            list_entity.setdefault(RDFS.CURIE.comment, message.TYPE_LIST_COMMENT%type_label)
            ...
            # group_entity.setdefault(RDFS.CURIE.comment,       ref_field_comment)
            # group_entity.setdefault(ANNAL.CURIE.record_type,  field_entity_type)
            # if not group_entity.get(ANNAL.CURIE.group_fields, None):
            #     group_entity[ANNAL.CURIE.group_fields] = (
            #         [ { ANNAL.CURIE.field_id:           entitytypeinfo.FIELD_ID+"/"+field_entity_id
